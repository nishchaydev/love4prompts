  ...
  const handleGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/tools/instagram-caption', {
        ...
        body: JSON.stringify({ description, tone, styles }),
      });
      ...
      <div className="relative z-10 container mx-auto max-w-[1200px] grid grid-cols-1 md:grid-cols-12 border-l border-[#cfc4c5] min-h-[calc(100vh-80px)]">
        ...
        <div className="hidden md:block md:col-span-3 lg:col-span-2 border-r border-[#cfc4c5] relative overflow-visible bg-transparent">
          <div className="p-8 sticky top-24 h-full flex flex-col justify-between">
            {/* Rotated CAPTION title reading bottom to top, C at top, centered in sidebar */}
            <h1
        ...
        <div className="md:col-span-9 lg:col-span-10 flex flex-col gap-10 p-6 lg:p-16 pb-32">
          ...
          <div className="flex flex-col items-start gap-4">
            ...
            <p className="text-base font-bold text-gray-700 max-w-2xl">
              Describe your photo and let our AI craft the perfect Instagram caption, complete with emojis and relevant hashtags.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div className="bg-white shadow-[8px_8px_0_rgba(0,0,0,1)] border-[3px] border-black rounded-3xl p-6 flex flex-col gap-6">
              <form onSubmit={handleGenerate} className="flex flex-col gap-6">
                    ...
                    {['Funny', 'Professional', 'Inspirational', 'Edgy', 'Storytelling', 'Short'].map((t) => {
                      ...
                      return (
                        <motion.button
                          ...
                          onClick={() => setTone(t)}
                          disabled={loading}
                          className={`py-2.5 px-2 text-center rounded-xl text-[11px] font-semibold tracking-tight cursor-pointer ${
                            isActive
                ...
                {/* Emojis Toggle */}
                <div className="flex items-center justify-between bg-gray-50 border-[2px] border-black shadow-[4px_4px_0_rgba(0,0,0,1)] p-4 rounded-2xl">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[13px] font-semibold text-black">Include Emojis?</span>
                    <span className="text-[10px] text-black/30">Add relevant emojis to your caption</span>
                  </div>
                  <motion.button
                    whileTap={{ scale: 0.9 }}
                    transition={springs.bouncy}
                    ...
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      includeEmojis ? 'bg-[var(--color-primary)]' : 'bg-black/10'
                    }`}
                  >
                    <span
                      className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                        includeEmojis ? 'translate-x-5' : 'translate-x-0'
                      }`}
                    />
                  </motion.button>
                </div>
                ...
                <div className="flex items-center justify-between bg-gray-50 border-[2px] border-black shadow-[4px_4px_0_rgba(0,0,0,1)] p-4 rounded-2xl">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[13px] font-semibold text-black">Include Hashtags?</span>
                    <span className="text-[10px] text-black/30">Append hashtags to maximize reach</span>
                  </div>
                  <motion.button
                    whileTap={{ scale: 0.9 }}
                    transition={springs.bouncy}
                    ...
                    disabled={loading}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      includeHashtags ? 'bg-[var(--color-primary)]' : 'bg-black/10'
                    }`}
                  >
                    <span
                      className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                ...
                <motion.button
                  ...
                  className="w-full h-12 rounded-2xl bg-[var(--color-primary)] hover:bg-[#ff5270] disabled:bg-black/5 disabled:text-black/20 text-white font-bold text-sm tracking-wide shadow-[4px_4px_0_#000] border-[2px] border-black flex items-center justify-center gap-2 cursor-pointer"
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-4.5 h-4.5 animate-spin text-black" />
                      <span className="text-black">Generating Caption...</span>
                    </>
                  ) : (
